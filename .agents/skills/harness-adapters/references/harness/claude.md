...
That section also owns how a checked profile's config directory reaches the launch and trust registration, what the preflight prints, and the per-pool Bypass Permissions step the login-only preflight does not cover.
