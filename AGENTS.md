...
config/bridge-vessel  Bridge inbox vessel name; LOCAL, gitignored; absent disables Bridge inbox scans
...
state/               volatile runtime signals; gitignored
  ...
  fork-sync.last-run   epoch of the curator vessel's last completed three-day fork comparison
  fork-sync.pending   persisted FORK_SYNC diagnostic and bounded patch-review list; cleared when the curated fork contains upstream
  fork-sync.stuck   persisted FORK_SYNC_STUCK line when the curated-fork comparison cannot complete
...
The locked bootstrap step performs the per-vessel AXI-suite currency check described in `docs/configuration.md` "AXI-suite self-update": patch and minor releases self-update, while `AXI_SUITE_REVIEW:` and `AXI_SUITE_STUCK:` require handling through `bootstrap-diagnostics`.
This vessel never calls Bridge from the updater; firstmate owns any stuck-status relay and dispatches a crewmate for project writes.
...
4. `check:` a per-task poll fired (usually a merge, Bridge inbox traffic, or X mode when enabled); act on it.
...
Two further PreToolUse seatbelts guard every primary shell tool call before it runs, independent of the pull-based banner: `bin/fm-arm-pretool-check.sh` blocks an unsafe watcher-arm invocation (`docs/arm-pretool-check.md`), and `bin/fm-cd-pretool-check.sh` blocks a persistent top-level `cd`/`pushd`/`popd` that would relocate the primary shell out of the firstmate home (`docs/cd-guard.md`).
...
The fleet's actual template is the curated fork, which merges the real upstream every three days and re-evaluates its fork-only patches through `bin/fm-fork-sync-check.sh` and `docs/fork-patches.md`.
Fork `main` must only advance without rewriting history: rebase and force-push are permanently forbidden, and every upstream-sync PR must land as a true merge commit.
