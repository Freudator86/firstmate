...
- `project-management` - load before adding, creating, removing, or initializing a project.
  Cloning or registering a project is add intake and uses the same trigger.
- `vessel-file-relay` - load when the captain hands over a local file path and asks that it be sent to, or made available to, a vessel, and when a secret or credential has to move from his machine into a vessel's host-scoped store.
