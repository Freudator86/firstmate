...
A lean request that asks only the rule Choice matched the full request (rule, profile, and status) on all 25 briefs; later router-axis support kept the same authority boundary by adding classifier questions only as evidence and keeping every gate in code.
...
It proves the classifier evidence line is emitted, the default effort recommendation remains `medium` for a clear ordinary objective, low-confidence classifier axes return `ambiguous` without a `profile:` line, and sensitive or explicit escalation classifications return `escalate` without directly authorizing a launch or sensitive action.
