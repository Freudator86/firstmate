...
The locked session-start bootstrap runs `bin/fm-axi-suite.sh`, which checks the npm-published `quota-axi`, `gh-axi`, `tasks-axi`, `gnhf`, `lavish-axi`, and `chrome-devtools-axi` suite (overridable via `FM_AXI_SUITE_TOOLS`) at most once every 24 hours per firstmate home.
The standalone script accepts `--force` for a scheduled or manual recheck and `--check-only` for a non-mutating report: an eligible-but-not-installed patch/minor update prints `AXI_SUITE_UPDATE:` instead of being installed, and a pending hook-setup retry for an already-current tool reports `AXI_SUITE_STUCK: ... retry pending` instead of being resolved.
Patch and minor releases install automatically into the npm prefix of the command currently on `PATH`; tools with hook integration rerun their hook setup afterward.
Major releases and suite commands not yet installed emit `AXI_SUITE_REVIEW:` and wait for captain approval.
Successful automatic changes emit `AXI_SUITE_UPDATED:`.
Registry, version-probe, permission, install, and hook failures emit `AXI_SUITE_STUCK:` and persist the same signal in `state/axi-suite-update.stuck` until a successful check clears it.
The mechanism never contacts Bridge; the `bootstrap-diagnostics` playbook owns escalation of a stuck vessel.
`FM_AXI_SUITE_CHECK_INTERVAL` overrides the cadence in seconds, with `0` checking every invocation, and `FM_AXI_SUITE_DISABLE=1` is reserved for tests or emergency diagnosis.
The whole suite's cumulative registry lookup, install, and hook-setup time is bounded by `FM_AXI_SUITE_NETWORK_TIMEOUT` seconds (default 30), not per tool, so a slow or unreachable npm registry cannot multiply that stall across every tool in the suite and cannot stall locked bootstrap beyond that one bound.
...
`bin/fm-watch.sh` reads the configured vessel's `inbox/<vessel>/new/` tree from the Bridge clone's fetched `origin/main`, so pending unacknowledged Bridge envelopes become durable actionable `check:` wakes without the watcher ever acknowledging or otherwise mutating envelope state.
`FM_BRIDGE_VESSEL` selects the vessel name when set to a non-empty value; an unset or empty variable falls through to the single value in the local, gitignored `config/bridge-vessel` file.
When neither is set, Bridge inbox scanning is disabled and silent so an unconfigured home never reads another vessel's inbox.
`FM_BRIDGE_ROOT` (default `$FM_HOME/projects/coditan-bridge`) selects the clone root; a missing Bridge clone is silent and costs one plain bash existence check per poll, no fork.
...
`state/.bridge-surfaced` records the last surfaced tree signature after its wake is durably enqueued, so an unchanged held inbox is surfaced only once; the marker is cleared when acknowledgment empties the inbox so a later byte-identical re-delivery fires normally.
...
FM_AXI_SUITE_CHECK_INTERVAL=86400   # seconds between AXI-suite currency checks per FM_HOME; 0 checks every invocation
...
FM_BRIDGE_VESSEL=         # optional override for config/bridge-vessel; when both are absent, Bridge inbox scanning is disabled
