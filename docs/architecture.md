...
The watcher's Bridge inbox check needs no opt-in: it reads this vessel's own `projects/coditan-bridge/inbox/coditan/new/` directory read-only on every poll (`FM_BRIDGE_VESSEL`/`FM_BRIDGE_ROOT` override the vessel and clone root), surfaces pending unacknowledged envelopes as a `check:` wake without ever acknowledging or mutating Bridge, and is silent when the clone or inbox is absent or empty.
