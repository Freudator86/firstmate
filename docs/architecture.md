...
The watcher's Bridge inbox check needs no opt-in: it bounded-fetches and reads this vessel's `inbox/coditan/new/` tree from the Bridge clone's `origin/main` remote-tracking ref (`FM_BRIDGE_VESSEL`/`FM_BRIDGE_ROOT` override the vessel and clone root), surfaces pending unacknowledged envelopes as a `check:` wake without ever acknowledging or mutating envelope state, and is silent when the clone or inbox is absent or empty.
