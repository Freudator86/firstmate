...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling, an X-mode mention, or pending Bridge inbox traffic, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The watcher's Bridge inbox check is per-home opt-in: when a vessel is configured (a non-empty `FM_BRIDGE_VESSEL`, falling through to the local, gitignored `config/bridge-vessel` file), it bounded-fetches and reads that vessel's `inbox/<vessel>/new/` tree from the Bridge clone's `origin/main` remote-tracking ref (`FM_BRIDGE_ROOT` overrides the clone root), surfaces pending unacknowledged envelopes as a `check:` wake without ever acknowledging or mutating envelope state, and is silent when the vessel is unconfigured or the clone or inbox is absent or empty.
Each pending inbox tree signature is surfaced once: after the wake is durably enqueued, the surfaced signature is recorded in `state/.bridge-surfaced`, an unchanged held inbox is silently absorbed on later polls, and the marker is cleared when acknowledgment empties the inbox so a later byte-identical re-delivery still fires.
Only that check's own cadence reacts to Bridge priority - tightening from `FM_CHECK_INTERVAL` to `FM_BRIDGE_URGENT_CHECK_INTERVAL` while the highest pending priority is `high` or `immediate` - leaving PR-merge polling, X-mode polling, and heartbeat cadence untouched; see `docs/configuration.md` ("Bridge inbox check") for the full signature-caching and bounded-execution mechanics.
...
Two further PreToolUse seatbelts complement that pull-based and push-based pair by blocking a narrow class of primary shell commands before they run: `bin/fm-arm-pretool-check.sh` denies an unsafe watcher-arm invocation ([arm-pretool-check.md](arm-pretool-check.md)), and `bin/fm-cd-pretool-check.sh` denies a persistent top-level `cd`/`pushd`/`popd` that would relocate the primary shell out of the firstmate home ([cd-guard.md](cd-guard.md)).
