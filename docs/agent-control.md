...
1. **Resolve the profile.**
   ...
   A claude task keeps the Claude capacity pool its record names, so a relaunch never moves a worker to another Anthropic account on its own; `--claude-profile <id>` moves it to a different locally configured pool deliberately, and naming one while relaunching onto a non-Claude harness refuses.
   The selected pool is preflighted in this step, so an unconfigured or unauthenticated pool refuses with the running agent untouched rather than after it has been stopped.
2. **Safe checkpoint.**
...
- A Claude capacity pool that this home has not configured, or whose login has lapsed, is refused **before** the running agent is stopped, so the task keeps its agent and the operator can retry with `--claude-profile` naming a ready pool.
