...
- A Claude capacity pool that this home has not configured, or whose login has lapsed, is refused **before** the running agent is stopped, so the task keeps its agent until that pool's login is restored.
