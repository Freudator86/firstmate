...
The seeded remote charter names that same host-local inbox, so the mate acts on the record the doorbell rang for instead of an unreachable inbox path in the parent's home; re-running `fm-remote-home-seed.sh` republishes that charter into an existing home.
