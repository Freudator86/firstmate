...
EOF
  ...
  assert_contains "$out" "WATCHER DAEMON DOWN - SUPERVISION IS OFF" "read-only guard did not surface watcher-daemon alarm"
...
EOF
  ...
  # Do this through an incompatible fake tool instead of deleting a fakebin
  # entry, because the host's BASE_PATH may still provide tools like node.
  cat > "$fakebin/no-mistakes" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' 'no-mistakes version v1.31.1 (fake)'
  exit 0
fi
exit 0
SH
